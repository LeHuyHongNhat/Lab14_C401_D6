...
<<<<<<< HEAD
        ...
        agent_cost = response.get("cost_usd", 0.0) if isinstance(response, dict) else getattr(response, "cost_usd", 0.0)
        judge_cost = judge_result.get("cost_usd", 0.0) if isinstance(judge_result, dict) else getattr(judge_result, "cost_usd", 0.0)
...
=======
            ...
            # Extract cost if available (handle dict or object)
            agent_cost = response.get("metadata", {}).get("cost_usd", 0.0) if isinstance(response, dict) else getattr(response, "cost_usd", 0.0)
            judge_cost = judge_result.get("cost_usd", 0.0) if isinstance(judge_result, dict) else getattr(judge_result, "cost_usd", 0.0)
...
>>>>>>> origin/main
    ...
    async def run_all(self, dataset: List[Dict]) -> Tuple[List[Dict], Dict]:
        ...
        avg_latency = sum(r.get("latency", 0.0) for r in results) / len(results) if results else 0.0
